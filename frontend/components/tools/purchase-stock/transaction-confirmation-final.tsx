      ...
      <CardContent className="space-y-4">
        <div className="rounded-md border border-green-200 bg-green-50 p-4">
          ...
          <div className="grid grid-cols-2 gap-2 text-sm">
            <p className="font-medium text-green-700">代码：</p>
            <p className="font-bold text-green-900">{ticker}</p>
            <p className="font-medium text-green-700">公司：</p>
            <p className="text-green-900">{companyName}</p>
            <p className="font-medium text-green-700">数量：</p>
            <p className="text-green-900">{quantity} 股</p>
            <p className="font-medium text-green-700">每股价格：</p>
            <p className="text-green-900">${maxPurchasePrice?.toFixed(2)}</p>
          </div>
        </div>
        <div className="rounded-md border border-green-300 bg-green-100 p-4">
          <p className="font-semibold text-green-800 text-lg">总费用：</p>
          <p className="font-bold text-2xl text-green-900">
            ${(quantity * maxPurchasePrice)?.toFixed(2)}
          </p>
        </div>
        <p className="text-center text-green-600 text-sm">
          已成功购入 {quantity} 股 {companyName}（{ticker}）。
        </p>
      </CardContent>
    </Card>
  );
